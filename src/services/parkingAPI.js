...
function buildStreetName(attributes) {
  ...
  const street = attributes.STREET || attributes.STREET_NAME || attributes.ST_NAME || attributes.ROAD || attributes.NAME || attributes.street || attributes.road || 'Unknown Street';
  const fromStreet = attributes.FROM_STREET || attributes.FROM_ST || attributes.FROM || attributes.FROM_ROAD;
  const toStreet = attributes.TO_STREET || attributes.TO_ST || attributes.TO || attributes.TO_ROAD;
...
function determineZoneType(attributes) {
  ...
  const costText = String(attributes.PARKING_COST || attributes.PARKING_COST || '').toUpperCase();
  const category = String(attributes.CATEGORY || '').toUpperCase();
  const subcategory = String(attributes.SUBCATEGORY || '').toUpperCase();
  const payment = String(attributes.PAYMENT_METHOD || attributes.METER_TYPE || attributes.PAY_DISPLAY || '').toUpperCase();
  const rate = parseFloat(attributes.MAX_RATE || attributes.RATE || 0) || 0;

  if (
    category.includes('NO PARKING') ||
    subcategory.includes('NO PARKING') ||
    costText.includes('NO PARKING') ||
    payment.includes('NO PARKING')
  ) {
    return 'no-parking';
  }
  ...
  if (
    ...
    subcategory.includes('PERMIT') ||
    payment.includes('PERMIT')
  ) {
    return 'permit';
  }

  if (costText.includes('FREE')) {
    return 'free';
  }

  if (
    costText.includes('PAID') ||
    payment.includes('METER') ||
    payment.includes('PAY') ||
    rate > 0 ||
    costText.includes('$')
  ) {
    return 'paid';
...
function getGeometryCenter(geometry) {
  ...
  switch (geometry.type) {
    ...
    case 'Polygon':
      return average(geometry.coordinates[0] || []);
    case 'MultiPolygon':
      return average((geometry.coordinates[0] && geometry.coordinates[0][0]) || []);
    default:
      return null;
  }
}
